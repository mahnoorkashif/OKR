...
extension Animation9 {
    private func createPaths() {
        let diff = Int((self.view.frame.width) / 100)
...
//            layer.strokeColor   = UIColor.black.cgColor
    ...
    private func createStar() -> UIImageView {
        let imageView = UIImageView(frame: CGRect(x: points[count].x, y: points[count].y, width: 20, height: 20))
    ...
    @objc private func updateTimer() {
        count = Int.random(in: 0...(paths.count - 1))
        let starView = createStar()
        self.view.addSubview(starView)
        createPositionAnimation(path: paths[count], starView: starView, duration: 5)
    }
}
